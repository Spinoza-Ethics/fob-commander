...
// Check cooldown
private _lastVehicleDropTime = missionNamespace getVariable ["lastVehicleDropTime", -9999];
private _now = time;
if (_now - _lastVehicleDropTime < 600) exitWith {
    private _remaining = 600 - (_now - _lastVehicleDropTime);
    player sideChat format ["Vehicle Drop cooldown: %1 seconds remaining.", ceil _remaining];
};

// Deduct credits and set cooldown
[-2] execVM "addCredits.sqf";
missionNamespace setVariable ["lastVehicleDropTime", _now, true];
...
private _ehID = addMissionEventHandler ["MapSingleClick", {
    ...
    private _supplyBoxType = "C_Offroad_01_F"; // Type of vehicle
    ...
    ["High Command", "Vehicle drop request received! ETA 50 seconds."] remoteExec ["BIS_fnc_showSubtitle", 0, true];
    ...
    [_helicopter, _supplyCrate, _dropPosition, _exitPos, _dropHeight] spawn {
        ...
        ["High Command", "Vehicle drop deployed!"] remoteExec ["BIS_fnc_showSubtitle", 0, true];
        ...
        ["High Command", "Vehicle drop has landed!"] remoteExec ["BIS_fnc_showSubtitle", 0, true];
