//THIS SCRIPT IS ONLY RUN ON NEWGAME START.
//SAME AS FOB SYSTEM SCRIPT BUT WITH LIMITED FEATURES.
//CHANGES MADE HERE WILL NOT EFFECT ANY PLAYER BUILT FOB... ONLY THE STARTING FOB WILL BE AFFECTED.

//newgame init delay (NEEDED)
sleep 7;
...
//========================================//
//       Automatic FOB Deployment         //
//========================================//
private _fobBuildPos = getPos player;

private _nearbyRedFlags = nearestObjects [_fobBuildPos, ["Flag_Red_F"], 300];
if (count _nearbyRedFlags > 0) exitWith {
    player sideChat "Cannot build F.O.B. too close to an enemy flag (within 300m). Capture the enemy flag first.";
};

private _nearbyFlags = nearestObjects [_fobBuildPos, [_flagType], 2000];
if (count _nearbyFlags > 0) exitWith { player sideChat "Can not build F.O.B.s within (2000m) of each other"; };
...
while {!_spawned && _spawnAttempts < _maxAttempts} do {
    ...
    private _safePos = [_fobBuildPos, _minDistance, _maxDistance, 5, 0, 0.3, 0, [], _fobBuildPos] call BIS_fnc_findSafePos;
    ...
    private _buildTime = 2; // This variable is declared but not used.
    ...
    private _flag = nil;
    {
        if (_x isKindOf _flagType) then { _flag = _x };
    } forEach _fobObjects;
    ...
    if (!isNil "_flag") then {
        ...
        _vehicleSign addAction [
            "<t color='#FFA500' size='1.2'>Request FOB Mission</t>",
            {
                ...
                [_selectedFOBMissionData, _caller] call fnc_executeMission;
                playSound "missionBrief";
            },
        ...
        _flag addAction ["<t color='#FFA500' size='1.2'>Request Air Support Radio</t> <t color='#00FF00'>(Cost 1 Field Credits)</t>", {
            ...
            // Cooldown logic for Air Support Radio
            missionNamespace setVariable ["fob_canSpawnAirSupportRadio", {
                private _lastAirSupportRadioTime = missionNamespace getVariable ["lastAirSupportRadioSpawnTime", -9999];
                private _now = time;
                if (_now - _lastAirSupportRadioTime > 600) then { missionNamespace setVariable ["lastAirSupportRadioSpawnTime", _now, true]; true } else { false };
            ...
            // Create radio bag with proper positioning
            private _radioBag = createVehicle [_radioClass, _spawnPos, [], 0, "NONE"];
            _radioBag setPosATL _spawnPos;
            _radioBag setDir (getDir _target + 90); // Face perpendicular to flag
            ...
            // Multiple delays to ensure proper initialization
            sleep 0.1;
            waitUntil { simulationEnabled _radioBag };
            sleep 0.2;

            // Verify the radio bag still exists and is accessible
            if (!isNull _radioBag && alive _radioBag && local _radioBag) then {

                // Add actions with different approach
                [_radioBag, _caller] spawn {
                    ...
                    // Wait and verify actions were added
                    sleep 1;
                    private _actionCount = count (actionIDs _radio);

                    // If still no actions, try direct method
                    if (_actionCount == 0) then {
                        sleep 0.5;
                    };
                };

                // Make the radio bag persistent and trackable
                ...
                // Refund credits if creation failed
        ...
        _flag addAction ["<t color='#FFA500' size='1.2'>Recruit AI Squadmate</t> <t color='#00FF00'>(Cost 1 Field Credits)</t>", {
                ...
                if (count _recruitAISubmenuActions > 0) then {
                    ...
                    // Add NATO Fire Team option first
